...
//		Uri dataUri = ContactsContract.Data.CONTENT_URI;
		...
		for(ContactsInfo info : infos){
			...
			Log.i("--tom", "add contact:-->" + info.getMobile());
	...
	/*删除所有联系人
	 * 
	 * type类型 0 手机联系人 1 sim卡联系人
	 * 
	 * 
	 * */
	public static void deleteAllPhone(Context context,int type) {
		...
		for(int i = 0;i < list.size();i++){
			...
			if(!entity.isChecked()){				
				...
				if(type == 1){
					 String where = "tag='" + entity.getName() + "'";  
					         where += " AND number='" + entity.getPhoneNum() + "'";  
